...
from core import schemas, settings
...
def get_item(item_id: int):
    with sync_session() as session:
        item = session.execute(select(Item).filter(Item.id == item_id)).scalar()
...
def update_item(
    ...
    with sync_session() as session:
        session.execute(
            update(Item).filter(Item.id == item_id).values(**update_data.dict())
        )
        session.commit()
...
def delete_item(item_id: int):
    with sync_session() as session:
        session.execute(delete(Item).filter(Item.id == item_id))
        session.commit()
...
def add_items(
    ...
    to_add = []
    for item in items:
        to_add.append(Item(**item.dict(exclude_none=True)))
    with sync_session() as session:
